...
# Función que crea las variables que se representarán 
# en el gráfico
mkdb <- function(pars) {
  # Comprueba que se ha especificado semilla para generador 
  # de números aleatorios
  if (!is.null(pars$seed))
    set.seed(pars$seed)

  # Variable x equiespaciada
  x <- seq(pars$rangex[1], pars$rangex[2], 
           length.out = pars$n)
  ...
  # Devuelve base de datos con las variables generadas
  tibble(y, x, u)
}

base_plot <- function(data, mapping) {
  ggplot(data = data, mapping = mapping) +
    theme_classic() +
    theme(axis.title.x =
            element_text(family = "Georgia", face = "italic",
                         size = 15, angle = 0, hjust = 1,
                         margin = 
                           margin(t = 0.08, unit = "inches")),
          axis.title.y =
            element_text(family = "Georgia", face = "italic", 
                         size = 15, angle = 0, hjust = 1,
...
mkplot <- function(data, pars, shades = NULL) {
  # Reserva espacio en blanco en los márgenes del
  # gráfico
  ...
  # Zonas sombreadas
  if (is.null(shades))
    final_plot <- base_plot + geom_point(size = 0.3)
  else {
    # Determina qué puntos están dentro de cada una 
    # de las zonas sombreadas  
    in_shades <- 
      reduce(map(shades, 
                 ~ between(db$x, .x[1], .x[2])), 
             `|`)
    # Estilo semitransparente para los puntos fuera 
    # de las zonas sombreadas
    final_plot <- base_plot +
      geom_point(data = filter(data, !in_shades), 
                 alpha = 0.1, size = 0.3) +
      geom_point(data = filter(data, in_shades), 
                 size = 0.3)
    # Zonas sombreadas
...
#|   - "Homoscedasticidad"
#|   - "Heteroscedasticidad"
#| fig-cap: Dispersión alrededor de la función de regresión poblacional
#| fig-width: 3
#| fig-height: 3
mkplot(db, pars)
mkplot(db_het, pars_het)
```
...
mkplot(db, pars, shades)
mkplot(db_het, pars_het, shades)
```
...
- [Halbert White](https://en.wikipedia.org/wiki/Halbert_White) mostró que se puede obtener una estimación consistente de $\var(\bhat)$ exista o no heteroscedasticidad:
  $$
    \widehat{\var}_R(\bhat_{1}) = 
    \frac{\sum(x_{i} - \bar{x})^{2} \uhat^2_i}{
      \Big[\sum(x_{i} - \bar{x})^{2}\Big]^{2}}.
  $$
...
- En la actualidad, se usa comúnmente la variante, debida a [Koenker](https://en.wikipedia.org/wiki/Roger_Koenker),
  del contraste que propusieron [Breusch](https://en.wikipedia.org/wiki/Trevor_S._Breusch) y [Pagan](https://en.wikipedia.org/wiki/Adrian_Pagan).
...
[White](https://en.wikipedia.org/wiki/Halbert_White) demuestra que la heteroscedasticidad sólo es problemática si la varianza condicional de $u$ depende de:
...
[Wooldridge](https://en.wikipedia.org/wiki/Jeffrey_Wooldridge) propone una variante del contraste de White donde sólo se incluyen en la regresión auxiliar las predicciones de MCO, $\yhat$, y sus cuadrados, $\yhat^2$:
$$
...
Aquí iba la figura @fig-het-shades-1.
...
Aquí iba la figura @fig-het-shades-2.
