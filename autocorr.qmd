...
Las propiedades de insesgadez y consistencia no dependen del supuesto de
...
-   Las fórmulas de MCO para estimar los errores típicos de $\bhat_j$ no
    son válidas en presencia de autocorrelación.
...
-   Si los **regresores son estrictamente exógenos** se pueden omitir
    las variables explicativas de la regresión auxiliar y basar el
    contraste en la regresión auxiliar:
    $$
      \uhat_{t} = \theta_{0} +
      \rho_1 \uhat_{t-1} + \dots + \rho_p \uhat_{t-p}
      + \text{error}.
...
-   Finalmente escribimos el modelo transformado como:
    ...
    donde $\tilde{y}_{t} = y_{t} - \rho y_{t-1}$ y $\tilde{x}_{t} =
     x_{t} - \rho x_{t - 1}$ son las variables en **cuasi diferencias**
    y $\tilde{x}_{0t} = (1 - \rho)$.
