# El estimador MCO con series temporales
...
-   En un **modelo estático**, un cambio en una variable explicativa
    sólo tiene efectos en el periodo en que se produce y no tiene
    influencia en los periodos posteriores.
...
-   Un **modelo autorregresivo con retardos distribuidos**,
    ...
    $$
    donde
    $\Exp(u_t | z_t, y_{t-1}, z_{t-1}, y_{t-2}, z_{t-2}, \dots) = 0$.
...
En la muestra ninguna de las variables explicativas es constante ni una
combinación lineal perfecta de las otras explicativas.

### Media condicional nula

**ST.3**: **Media condicional nula**.

La media del término de error es $0$ condicional a los valores que toman
las explicativas en **todos** los periodos de tiempo :
$$
...
-   **Eficiencia**: si, además, se cumplen **TS.4** y **TS.5**. En este
    caso, se pueden usar las fórmulas de MCO de los errores típicos de
    $\hat{\beta_j}$.
...
-   Las variables explicativas $\bm{x}_t$ son **contemporáneamente
    exógenas**:
    $$
      \Exp(u_{t}|\bm{x}_{t}) = 0.
    $$
...
-   **Normalidad asintótica**: si además se cumplen **TS.4\'** y
    **TS.5\'**.

### Conclusiones

-   Con muestras de tamaño moderado o grande, bastan los supuestos
    **ST.1\'** a **ST.5\'** para garantizar buenas propiedades del
    estimador MCO.
...
-   También es frecuente, especialmente en modelos financieros, la
...
-   Las aportaciones de Newey y West, y de autores posteriores, muestran
    ...
    requieren los supuestos **ST.1\'**, **ST.2\'** y **ST.3\'** y que
    son válidas aun en presencia de correlación serial y
    heteroscedasticidad.
...
-   Se estiman por MCO los parámetros del modelo de regresión. MCO es
    consistente si se cumplen los supuestos **ST.1\'**, **ST.2\'** y
    **ST.3\'**.
